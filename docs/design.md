...
- **Sources** produce raw events: keyboard, mouse (click, scroll, drag), PTY output, filesystem changes, timers.
- **Events** are values that describe what happened. They carry no logic.
- **Dispatch** maps events to state mutations. Keyboard events resolve through keybindings to actions. Mouse events are hit-tested against grid cell rects to determine the target widget — clicks select/focus, scroll wheel scrolls the widget under the cursor. Background events (PTY, filesystem) are drained from channels.
...
### State layer
Knows about domain concepts. Does not know about UI layout or rendering. Exposes query methods for reading and mutation methods for writing.
...
### I/O layer
Knows about files, processes, and the network. Does not know about UI or domain logic. Provides channels for async communication with the main loop.
...
3. **Minimize allocations in the render path.** Batch similar items. Reuse buffers where possible. The hot path is render → every 16ms — it must be cheap.
4. **Background threads for blocking I/O.** The main thread never blocks on a read, write, or process spawn.

## Error Handling

- **Recoverable errors** (file not found, process spawn failure) are reported to the user via the UI. The application continues.
- **Unrecoverable errors** (terminal setup failure) cause a clean exit with an error message.
